...
export async function scrapeDailyStatsLeaders() {
  ...
  const data = await page.evaluate(() => {
    const categories = Array.from(
      document.querySelectorAll("div.data_grid_box")
    );
    const results: any[] = [];
    ...
    for (const category of categories) {
      ...
      const subject = subjectEl
        ? subjectEl.getAttribute("data-tip")
        : "Unknown";

      const rows = Array.from(
        category.querySelectorAll("table.columns tbody tr")
      );
      const players = [];
      ...
      for (const row of rows) {
        ...
        const playerName =
          row.querySelector("td.who a")?.textContent?.trim() || "";
        const team =
          row.querySelector("td.who .desc")?.textContent?.trim() || "";
